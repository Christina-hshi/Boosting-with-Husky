  ...
  namespace mllib{
      class Classifier{
          public:
              ...
              virtual AttrList<Instance, double>&  predict_proba(Instances& instances,int classnow,std::string prediction_name="predicted_probability")=0;
              // using the default method to convert probability to output label
              virtual AttrList<Instance, int>&  classify(Instances& instances,std::string prediction_name="predicted_label")=0;
